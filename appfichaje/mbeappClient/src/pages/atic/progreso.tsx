...
const ProgresoAtic = () => {
    const _itemSelected = 'atic_progreso'
    ...
    useEffect(() => {
        // Cargar datos cuando el componente se monta o cuando userData cambia
        fetchEmployeeData()
    }, [userData])
    ...
    const fetchEmployeeData = async () => {
        try {
            ...
            // Obtener los datos reales del usuario (userData es una función getter)
            const currentUserData = typeof userData === 'function' ? userData() : userData

            // DEBUG: Ver exactamente qué datos del usuario tenemos
            console.log('=== DEBUG USER DATA COMPLETO ===')
            console.log('currentUserData completo:', currentUserData)
            console.log('currentUserData.id:', currentUserData?.id)
            console.log('currentUserData.username:', currentUserData?.username)
            console.log('currentUserData.email:', currentUserData?.email)
            console.log('currentUserData.nombre_completo:', currentUserData?.nombre_completo)
            console.log('currentUserData.nombre:', currentUserData?.nombre)
            console.log('currentUserData.apellido:', currentUserData?.apellido)
            console.log('currentUserData.departamento:', currentUserData?.departamento)
            console.log('currentUserData.rol:', currentUserData?.rol)
            console.log('currentUserData.roles:', currentUserData?.roles)

            // Listar TODAS las propiedades disponibles
            console.log('Todas las propiedades disponibles:', Object.keys(currentUserData || {}))

            // Determinar qué enviar al API
            let userIdentifier = ''

            // Prioridad 1: Si ya tenemos nombre_completo, usarlo directamente (sin consultar BD)
            if (currentUserData?.nombre_completo) {
                userIdentifier = currentUserData.nombre_completo
                console.log('Usando nombre_completo directamente:', userIdentifier)
            }
            // Prioridad 2: Usar username (para consultar la BD por username)
            else if (currentUserData?.username) {
                userIdentifier = currentUserData.username
                console.log('Usando username para consultar BD:', userIdentifier)
            }
            // Fallback: usar ID si no hay username
            else if (currentUserData?.id) {
                userIdentifier = currentUserData.id.toString()
                console.log('Usando ID como fallback:', userIdentifier)
            }
            // Fallback final: usar un usuario por defecto si no se puede identificar
            else {
                userIdentifier = 'Alex MBE' // Username por defecto para testing
                console.log('Usando fallback userIdentifier:', userIdentifier)
            }

            console.log('userIdentifier FINAL enviado al API:', userIdentifier)
            console.log('========================')
