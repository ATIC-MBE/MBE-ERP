...
`,
        ...
        let responseD = await this.client.execQueryPool( async (client): Promise<Array<IModel | IErrorResponse>> => {
            ...
            if ( dataDB && data.plataformas && data.plataformas.length !== 0 ) {
                ...
                queryData = {
                    ...
                    text: ` UPDATE ${Constants.tbl_plataforma_infopiso_myd_sql} SET estado = -1
                ...
                for (let i = 0; i < data.plataformas!.length; i++) {
                    const queryData = {
                        ...
                        text: ` UPDATE ${Constants.tbl_plataforma_infopiso_myd_sql} SET 
                                    link = $1, 
        ...
        const queryData = {
            ...
            text: `
                    ...
                    LEFT JOIN (
                        ...
                        LEFT JOIN ${Constants.tbl_plataforma_infopiso_myd_sql} pi ON (ipc.id = pi.idinfopisocom AND pi.estado = 1)
                        LEFT JOIN ${Constants.tbl_plataforma_comercial_myd_sql} pc ON (pc.id = pi.idplataformacom)
                        WHERE p.id = $2
                        GROUP BY p.id
                    ) plfc ON plfc.id = p.id
                    WHERE p.id = $2
        ...
        const queryData = {
            ...
            text: `
                    ...
                    LEFT JOIN (
                        ...
                        FROM ${Constants.tbl_info_piso_comercial_myd_sql} ipc
                        LEFT JOIN ${Constants.tbl_variables_reserva_myd_sql} vr ON (ipc.id = vr.idinfopisocom AND vr.estado <> -1)
                        LEFT JOIN ${Constants.tbl_tipo_estancia_myd_sql} te ON (te.id = vr.idtipoestancia)
    ...
    async updateVariableReserva(id: BigInt, data: IInfoPisoComercial): Promise<IInfoPisoComercial | IErrorResponse> {
        let responseD = await this.client.execQueryPool( async (client): Promise<Array<IModel | IErrorResponse>> => {
            ...
            queryData = {
                name: 'inactivar-plataformas',
                text: ` UPDATE ${Constants.tbl_variables_reserva_myd_sql} SET 
                        estado = -1, 
            ...
            const queryDataUpdate = {
                ...
                text: `UPDATE ${Constants.tbl_info_piso_comercial_myd_sql} SET 
                        ...
                        WHERE id IN (SELECT id 
                            FROM ${Constants.tbl_info_piso_comercial_myd_sql} 
                            WHERE estado = 1 AND 
                            idpiso = $3 
                            ORDER BY id DESC 
                            LIMIT 1)
    ...
    async insertByEstadoGeneral(data: IInfoPisoComercial): Promise<IInfoPisoComercial | IErrorResponse> {
        let responseD = await this.client.execQueryPool( async (client): Promise<Array<IModel | IErrorResponse>> => {
            ...
            const queryDataUpdate = {
                name : "update-info-piso-comercial",
                text: `UPDATE ${Constants.tbl_info_piso_comercial_myd_sql}
                        SET
                        estado_general = $1,
                        fecha_ultimo_cambio = $2,
                        idusuario_ult_cambio = $3
